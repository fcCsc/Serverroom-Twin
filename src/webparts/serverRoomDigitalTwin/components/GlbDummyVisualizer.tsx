...
const GlbDummyVisualizer: React.FC<IGlbDummyVisualizerProps> = ({ racks, devices, placements, selectedRackId, selectedDeviceId, mode, rackSide = 'front', onRackSelected, onDeviceSelected }) => {
  ...
  React.useEffect(() => {
    ...
    const addDevice = async (rackGroup: THREE.Group, rack: IInfraRack, placement: IRackPlacement, rackScaleY: number): Promise<void> => {
      ...
      recolorObject(block, color);
      block.scale.set(widthFactor(mountWidth) * (kind === 'small' ? 2 : 1), heightU, 1);
      block.position.set(laneX(mountWidth, placement.horizontalSlot), (placement.startU - 1) * unitHeight + (heightU * unitHeight / 2), -rackInnerDepth / 2 - 0.18);
      block.userData = { type: 'device', deviceId: device.id, rackId: rack.id };
      rackGroup.add(block);
      deviceClickTargets.push(block);

      const labelText = `${device.hostname}\n${device.type} · U${placement.startU}/${heightU}U${heightU > 1 ? `\n${device.vendor} ${device.model}` : ''}`;
      const label = makeLabel(labelText);
      label.position.set(block.position.x, block.position.y + (heightU / 2) + 0.34, -rackInnerDepth / 2 - 0.78);
      label.scale.set(mountWidth === 'Full' ? 2.15 : 1.25, heightU > 1 ? 0.72 : 0.48, 1);
      rackGroup.add(label);
    };
    ...
    const addRack = async (rack: IInfraRack, index: number): Promise<void> => {
      ...
      await Promise.all(rackPlacements.map((placement) => addDevice(rackGroup, rack, placement, rackScaleY)));
    ...
    const onPointer = (event: PointerEvent, click: boolean): void => {
      ...
      if (deviceHit) {
        const target = deviceHit.object.parent || deviceHit.object;
        const deviceId = target.userData.deviceId || deviceHit.object.userData.deviceId;
