...
const markPreviewReadyIfRendered = (): void => {
  const appRoot = document.getElementById('app-root');
  const hasDashboardText = !!appRoot && (appRoot.textContent || '').trim().length > 20;
  if (hasDashboardText) document.body.classList.add('previewReady');
};
...
try {
  ...
  window.setTimeout(markPreviewReadyIfRendered, 1200);
  window.setTimeout(markPreviewReadyIfRendered, 3000);
} catch (error) {
